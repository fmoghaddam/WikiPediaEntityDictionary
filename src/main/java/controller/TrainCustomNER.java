...
public class TrainCustomNER {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 5; i < positiveLines.size(); i++) {
			final String line = positiveLines.get(i);
		...
		//writeToFile(positiveResult, "NER/positive.txt");
		
		
		final List<Tuple> negativeResult = new ArrayList<>();
		for (int i = 5; i < negativeLines.size(); i++) {
			final String line = negativeLines.get(i);
			final String[] split = line.split("\t");
			String data = split[3];
		...
		//writeToFile(negativeResult, "NER/difficultNegative.txt");
		positiveResult.addAll(negativeResult);
		writeToFile(positiveResult, "NER/train.tsv");
	}
